...
- **正文标题是增量式的，跟真书排版一致**（2026-08 修订，取代早期"每个 block
  ...
  标题再深一级，`cli/renderBlocks.ts` 的 `indentContent`）——**必须手动按屏幕
  宽度断行再逐行加前缀，不能只给整段长文本的开头加一次前缀**：`content_md`
  里一段话本身不含换行符，只加一次前缀的话，终端自动折行的续接部分会不带
  缩进地贴回屏幕左边（曾经踩过这个坑）。代码块内部不参与断行，只整体加
  前缀；代码块的 ` ```scheme `/` ``` ` 围栏标记本身不打印——纯文本终端不解析
  Markdown，围栏杵在那里不高亮也不成框，纯属噪音（Windows 终端下用户实测
  反馈过）——换成一行纯文本提示 `Code (scheme):`（没有语言标注就是 `Code:`），
  上下各配一条 `---` 边界线，**宽度跟代码内容本身走（代码块里最长一行的
  长度），不拉满屏幕**——拉满屏幕会跟分页/分节的横线混淆，用户明确反馈过。
  实现见 `cli/renderBlocks.ts`、`docs/DESIGN.md` §7.4。
- **进度呈现** = 小节 + 整章百分比同时显示。
...
- **给终端输出的一整段长文本只在开头加一次缩进前缀是不够的**：`content_md` 里一个段落本身不含换行符，这一整行长到超过屏幕宽度时终端会自动折行，但折行续接的部分不知道这行本身带了缩进，会贴回屏幕左边（表现为"只有第一行缩进，后面又顶头了"）。凡是要给可能超出屏幕宽度的文本加缩进，必须自己先按 `屏幕宽度 - 缩进长度` 手动断行，再给断出来的每一行都加前缀，不能指望终端的自动折行认识这个缩进（见 `cli/renderBlocks.ts` 的 `indentContent`/`wordWrap`）。
- **不能假设"同一个 Project Gutenberg 书"内部结构都一样**：Gatsby 的 epub 是从
  ebookmaker 直接转 HTML 源生成的，靠 `<div id="chapter-N">` 分章节；西游记的 epub
  是从纯文本 `.txt` 源转换的，没有任何容器元素，章节标题跟 `*** START/END OF THE
  PROJECT GUTENBERG EBOOK ***` 样板标记都是裸 `<p>`/`<span>`，起止标记甚至不在
  `<p>` 里，得在原始字符串层面找子串裁剪，不能假设标签结构能帮上忙。加新书源之前
  一定要先解压实际检查内部 HTML 结构（`grep`/`find_all` 探一遍 tag 分布），不能照抄
  上一本书的 adapter 假设。另外：章节数字的书写在同一本书里都可能不统一（标准数字/
  逐位读数/用"○"这个符号代替十位的零都出现过），adapter 不要尝试解析数字本身的
  含义，按遇到标题的顺序自己编号更稳（`gutenberg_txt_adapter.py`）。
- **subagent 报告"已核对原文逐字引用"不能直接信**：西游记 8 个并行 subagent 都在
  自己的任务简报里被要求"explanation 里的引用必须核对原文"，也都在完成报告里声称
  做到了，但组装阶段自己写脚本抽查后发现 120 道题里约一半的"引用"其实是转述、不是
  原文子串。指令要求"验证过"不代表真的按字节比对过——凡是"逐字引用"这类可以机械
  验证的要求，最后要自己写一个自动化检查脚本核实，不能只看 agent 报告里的自我
  确认（尤其是多个 agent 并行、每个都独立复现同一种"用理解代替直接抄"的偏差时）。
