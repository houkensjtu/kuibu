...
## 2026-08-08 —— 乔布斯传铺开批次一：第 3–21 章（19 章）

用户拍板"把乔布斯传铺开到全书"，问了一轮节奏（一次做完 / 分两批中间验收 /
按 8-10 章一波但不强制停），选了**分两批**：批次一做第 3–21 章（约半本），
组装成可实际打卡的中间版本验收，通过再做批次二（第 22–41 章 + 尾声）。这条
milestone 跟 M3（西游记）一样是 review-gated——见 `docs/MILESTONES.md` M4。

**规模现实检查**：动手前先核实了机械切分产物（`pack-gen/build/private/sjobs/
sections/`），43 节（前言+41章+尾声）里只有前 3 节（前言/1/2）有内容，剩余
40 节共约 2015 段原文，按已有比例（~3.5 段/block）估算会产出 550-600 个新
block——是 Gatsby 全书或西游记前 10 回的 4-5 倍。这个规模差异是主动核实出来
用来跟用户对齐节奏预期的，不是凭感觉。

**新增质量关卡**（本次最重要的产出）：`pack-gen/scripts/verify_explanation_quotes.py`
——机械核对每道题 `explanation` 里"原文：""……"" 引用的每一段是否真是这一节
自己原文的精确子串，不依赖 subagent 自我声明"已核对过"（西游记 M3 那次的
教训：8 个并行 agent 都在报告里声称核对过，但组装后抽查发现约一半其实是
意译加引号）。写完先拿已批准的 foreword/1/2 当基准跑了一遍，抓出几类
**合法但会被朴素子串比对误报**的写作惯例，逐条修正检查逻辑而不是删掉检查：
- 引用嵌套引用时，内层从原文的"“ ”"换成"‘ ’"以避免跟外层撞——按引号规整
  后再比对。
- 中文人名/书名后紧跟的西文原名括注（如"阿兰·图灵(Alan·Turing)"、
  "《48小时》(48 Hours)"）引用时按惯例省略——先从源文里剥掉这类括注再比对
  （批次一执行过程中还发现括注可能以数字开头，如"(48 Hours)"，最初的正则
  只认字母开头漏掉了这种，已经改成字母/数字都认）。
- 引用在语义完整处截断收尾时，原文的"，"/"——"会被改写成句号（外面可能还
  嵌套着一层引号收尾符）——允许从右往左剥掉几个收尾标点字符后再比对。

这套关卡最后处于"能抓真问题、不误报合规写法"的状态：批次一实际执行中，
至少 3 个 agent 的报告明确写到检查脚本真的抓到了问题并促成了修正（例如
chapter 5 一道题最初把两段不相邻的引用之间的叙述性归属短语也囊括了进去，
被脚本拦下后改成用"……"跳过叙述、只留两段真引用）——不是摆设。

**内容生成**：19 章拆成 3 波并行 subagent（`Agent` 工具，`model: sonnet`），
Wave A（3-9，7 章）→ Wave B（10-16，7 章）→ Wave C（17-21，5 章，含
伊卡洛斯/NeXT/凡人三个体量是普通章节 3-4 倍的超大章节）。每个 agent 拿到
同一份任务简报：机械切分的段落原文、`section_prompt.md` 规格、已批准的
Chapter 1 范例、显式的引用格式规则、以及"必须实际跑通两项自检（pydantic
校验 + 引用核对脚本）才算完成，不能只在报告里自称做过"的要求。每一波完成
后，除了信任 agent 自己的自检，还额外独立跑了一遍 pydantic 校验 + 引用
核对 + 段落覆盖/索引越界检查，三波全部干净通过。19 章合计 279 blocks /
348 items（含 foreword/1/2 已有的 36，全书批次一小计 421/502/502）。

**Recap checkpoint（104 条，本次工作量最大的部分）**：重新跑
`compute-recap-boundaries.ts` 后发现一个意外结果——**前 9 条 checkpoint 的
边界这次完全没变**（跟 Gatsby 当年全书铺开时前 3 条边界因为后面多了内容而
往后挪不同），因为原来第 9 条恰好卡在"读到第 36 块（第二章末尾）"，新增的
第 3 章内容没有被贪心打包算法拉进来——这提醒了一件事：**"新增内容会不会
让早期 checkpoint 边界变化"取决于新内容加在旧边界之后是否还有富余的时间
预算，不能预设"肯定不变"或"肯定会变"，每次都要重算之后实际对比**。据此，
9 条已批准的 checkpoint 原样保留，只需要新写 95 条（checkpoint 10-104，
覆盖 block 37-421）。

这 95 条被拆成 4 组（10-33/34-57/58-81/82-104）交给 4 个并行 agent——这是
本次流程上跟 Gatsby 先例不同的一点：Gatsby 当年判断"回顾必须顺序写"没有
交给 agent，但实际核实 `recap_md` 的语义后发现**每条 checkpoint 之间并无
文本依赖**（不是滚动累积压缩前文，而是"这一段读了什么"的独立小结，查表用
的"累计 block 数"只是定位键，不是内容本身要累积），所以这次判断可以安全
并行，用已批准的 9 条当风格/长度校准范例。一个 agent 在报告里主动坦白
自己为了叙事流畅在两条 checkpoint 里略微软化了具体数字/巧合描述——已记录，
不算严重问题（`recap_md` 本来就是允许改写的复述，不适用引用核对那套逐字
规则），但值得留意这类"为了读起来顺口而丢失具体细节"的偏差长期会不会
累积。

**合并组装**：`build_all_sjobs_sections.py` 合并 43 节里已完成的 22 节
（前言+1-21）→ `421 blocks / 502 items / 502 questions / 0 exercises`；
104 条 recap checkpoint 合并进 `packs/private/sjobs/recap_checkpoints.json`；
两侧 schema 全量校验（Python pydantic `ContentPack` + TS `cli/loadPack.ts`
真实路径）+ `checkPackReferences` 交叉引用检查全部通过；`npm test`（216）+
pytest（69）保持全绿；用真实事件日志的一次性副本验证过 `kuibu status
--pack packs/private/sjobs`，已读的 4 个 block/streak/目录都正常，完读
百分比从"3 章的 1/9"正确变成"21 章的 1/43"，没有碰用户的真实打卡记录；
`npm run bundle -- sjobs` 刷新 `bundles/sjobs.kuibu.json`（421 blocks），
可以照 M5.5 那次的流程重新导入网页版。

**已知需要留意的点（记录、暂不处理）**：
- Chapter 20（凡人，涉及癌症诊断/私生活那一章）知识点密度明显偏高
  （36 blocks 对 61 items，接近 1.7 items/block，其余章节大多接近 1:1）——
  内容本身通过了两项自检，只是密度不均匀，将来如果觉得这一章问答太密可以
  单独精简，不算错误。
- 上面提到的 recap checkpoint 数字软化（r0022/r0024 附近），细节不算错但
  跟"逐字引用"那套严格标准比显得更松——`recap_md` 场景允许这样，只是记录
  下来避免以后跟别的字段的标准搞混。
- `verify_explanation_quotes.py` 目前只处理"原文：""……"" 这类 Latin 括注/
  截断句号的容错，还没遇到过的新括注/标点变体以后大概率还会需要继续补
  （历史上每一波新章节基本都会暴露至少一种没预见到的合法写法）。

版本号：批次一是"分两批验收"里的中间产物，还没构成用户可见的、可以自用的
完整能力（书还没读完），按 M3/M4 先例先不动版本号、`docs/MILESTONES.md`
M4 也不打勾，等批次二和全书组装完再一起判断。
