...
**进展（2026-08-08）**：用户拍板铺开全书，选择"分两批、中间验收一次"的节奏。
- [x] **批次一**：前言 + 第 1–21 章（约半本），19 个新章节靠 3 波并行 subagent
  完成，新增 `pack-gen/scripts/verify_explanation_quotes.py` 自动核对
  `explanation` 逐字引用（防重犯西游记 M3 的"号称核对过其实是意译"问题，
  过程中真的抓到并修正了几处）；104 条 recap checkpoint（前 9 条边界这次
  没变，沿用；新写 95 条，4 组并行完成）。合并组装：421 blocks / 502 items /
  502 questions，两侧 schema + 交叉引用校验通过，`npm test`/pytest 全绿，
  真实事件日志验证过续读不受影响，`bundles/sjobs.kuibu.json` 已刷新。
  已知细节（不算错误，记录见 `docs/JOURNAL.md` 2026-08-08 条目）：Chapter 20
  知识点密度偏高；个别 recap checkpoint 为叙事流畅略微软化了具体数字。
- [ ] **批次二**：第 22–41 章 + 尾声，同一套流程铺开，完成后跟批次一合成
  完整的 43 节全书包——**下一步**。

**验收**：批次二完成、全书组装好之后，用户实际读一段再决定是否通过；批次一
本身也还没经过用户的实际阅读验收，**不打勾**。
